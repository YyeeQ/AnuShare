...
public class ModerationToolsGetReportsTests {
	...
	public void tearDown() {
		// Clean up active reports through the public API only.
		removeReportsFor(oldestReported);
		removeReportsFor(mostReported);
		removeReportsFor(laterReported);
		removeReportsFor(unreported);
		removeReportsFor(otherPostReported);
	}
	...
	private static void retract(Message message, User user) {
		ModerationTools.removeReport(message.id(), user.id());
	}

	private void removeReportsFor(Message message) {
		if (message == null) return;
		ModerationTools.removeReport(message.id(), userA.id());
		ModerationTools.removeReport(message.id(), userB.id());
		ModerationTools.removeReport(message.id(), userC.id());
	}
}
